...
def _get_peaks(heatmap: torch.Tensor, kernel_size: int = 3):
    ...
    """
    pad = (kernel_size - 1) // 2
    max_pool = torch.nn.functional.max_pool2d(heatmap, kernel_size=kernel_size, stride=1, padding=pad)
...
def decode(
    ...
    for peak_id, peak_score, peak_class, offset, size in zip(peak_ids, peak_scores, peak_classes, offsets, sizes):
        ...
        center_xs = xs / downsample + offset[:, 0]
        center_ys = ys / downsample + offset[:, 1]
        ws, hs = size[:, 0], size[:, 1]
